...
<svelte:head>
  ...
  <meta
    name="description"
    content="oreohive human - a place for political commentary (a.k.a. deep stuff) and other toss you may or may not care about."
  />
</svelte:head>
    ...
    <ul>
      {#each [...data.documents] as post}
        ...
        >
          ...
          />
          <a href={`/human/human-articles/${post.uid}`}
            ><i>{post.data.title ?? "no title found!"}</i></a
          >
          <p>
            {post.data.date ?? "no article date found!"} by {post.data.author ??
