...
<svelte:head>
  ...
  <meta
    name="description"
    content="oreohive labs | perhaps not quite as scientific as it might sound."
  />
</svelte:head>
...
{#if data.posts && data.posts.length > 0}
  <div style="color: #7f7fbe;">
    {#each [...data.posts] as post}
      ...
      <div
        style="display: flex; border-style: dashed; margin-top: 0px; margin-bottom: 32px; border-radius: 12px;"
      >
        <div
          style="display: flex; border-radius: 12px; margin: 0px; margin-top: 18px;"
        >
          <Panel
            colour="#fcfcff"
            img_url={post.data.image.url ?? "/logo.webp"}
            href={`/labs/labs-articles/${post.uid}`}
          />
          <div>
            <a href={`/labs/labs-articles/${post.uid}`}
              ><h3>
                <PrismicRichText field={post.data.title ?? "a post title"} />
              </h3></a
            >
